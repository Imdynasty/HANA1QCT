...
import AnimatedNumber from "../AnimatedNumber2";
import DomesticBalanceList from "./DomesticBalanceListInOrder";
import CircularProgress from "@mui/material/CircularProgress";
// import axios from "axios";
import { AutoComplete, Select } from "antd"; // antd에서 AutoComplete, Select 불러오기
// import "antd/dist/antd.css"; // antd 기본 스타일

const { Option } = Select;

function DomesticOrder({ selectedStock }) {
  ...
  const [orderPopoverAnchorEl, setOrderPopoverAnchorEl] = useState(null); // 새로운 Popover의 anchor
  ...
  useEffect(() => {
    if (isBalanceModalOpen) {
      ...
      const fetchData = async () => {
        try {
          // 여기서 실제 데이터 요청 로직이 들어가야 합니다.
          // 예를 들어, axios를 사용해 데이터를 요청할 수 있습니다.
  ...
  const handlePriceChange = (value) => {
    const numericValue = value.replace(/[^0-9]/g, ""); // 숫자 이외의 문자는 제거
    const formattedValue = Number(numericValue).toLocaleString(); // 숫자에 쉼표 추가
    ...
    axios
      ...
      .then((response) => {
        if (response.data.customerPassword === password) {
          ...
          fetch("http://54.180.14.45:5000/domestic/order", {
            method: "POST",
            headers: {
  ...
  const handleMaxOrder = async (event) => {
    if (!price || parseInt(price.replace(/,/g, "")) === 0) {
    ...
    try {
      ...
      const totEvluAmt = response.data.totEvluAmt;

      const numericPrice = parseInt(price.replace(/,/g, ""), 10);
      const maxQuantity = Math.floor(totEvluAmt / numericPrice);
    ...
    <div className="order-form">
      ...
      <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
        <Tabs
          ...
          {/* <Tab sx={{ color: "black", fontSize: "17px" }} label="정정/취소" /> */}
        </Tabs>
      </Box>
      {activeTab < 2 && (
          ...
          <div className="infoorder">
                ...
                <div
                  ...
                  <div style={{ marginLeft: "10px" }}>
                    <IconButton
                      onClick={() =>
                        handlePriceChange(
                          (parseInt(price.replace(/,/g, "")) - 100).toString()
                        )
                      }
                      size="small">
                      <RemoveIcon />
            ...
            <div className="total-price">
              ...
              <span style={{ fontSize: "30px", fontWeight: "650" }}>
                <AnimatedNumber
                  value={(
                    quantity * parseInt(price.replace(/,/g, "") || 0)
                  ).toLocaleString()}
                />
              </span>
            </div>
            {activeTab === 0 && (
              <div style={{ marginBottom: "0px" }} className="total-price">
                <label>증거금</label>
                <Select
      ...
      <Modal
        ...
        <Box
          ...
          {isLoading ? (
            ...
            <DomesticBalanceList accountId="종합위탁" /> // 로딩 완료 시 데이터 표시
          )}
        </Box>
      </Modal>
    </div>
