...
import { Avatar, Button, List, AutoComplete, Skeleton, Divider } from "antd";
...
const SearchStock = ({ onSelectStock }) => {
  ...
  const [searchTerm, setSearchTerm] = useState("하나금융지주"); // 초기값 설정
  ...
  return (
    <Box className="multibox" sx={{ width: "100%" }}>
      <Popover
        open={popoverOpen}
        anchorEl={anchorEl}
            ...
            <Button
              ...
              style={{
                color: favoriteStocks.includes(selectedStock.stockCode)
                  ? "red"
                  : "gray",
                marginLeft: "10px", // Adjust the margin to align properly with search button
                fontSize: "20px",
              }}
            />
          )}
          ...
          <List
            ...
            renderItem={(item) => (
              <div>
                ...
                <AskingPrice stockCode={item.stockCode} />
              </div>
            )}
          />
        )}
