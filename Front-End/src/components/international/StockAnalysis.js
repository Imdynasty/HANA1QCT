    ...
    const fetchMatchingIndustries = async () => {
      try {
        const [topInvestmentResponse, favoriteIndustriesResponse] =
          await Promise.all([
            axios.get(
              "http://13.125.78.241:8081/overseas/analysis/top/종합위탁/top-investment"
            ),
            axios.get(
              "http://13.125.78.241:8081/overseas/analysis/1qct/favorite-industries"
            ),
          ]);
        ...
        const byCapitalizationResponse = await axios.get(
          "http://13.125.78.241:8081/hana/overseasstock/1qct/by-capitalization"
        );
        ...
        const priceDifferencePromises = filteredStocks.map((stock) =>
          axios.get(
            `http://13.125.78.241:8081/hanact/overseasstockprices/${stock.stockCode}/price-difference`
          )
        );
        ...
        const priceDifferencesData = priceDifferencesResponses.map(
          (res) => res.data
        );
    ...
    ? {
        ...
        yAxis: [
          ...
          },
          {
            ...
            labels: {
              formatter: function () {
                return `${this.value.toLocaleString("ko-KR")}USD`; // 콤마 구분 추가
              },
              style: {
                // color: "#6C7AD9",
              },
