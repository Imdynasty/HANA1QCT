...
const AmericaLogo = `${process.env.PUBLIC_URL}/img/america.png`;
...
function ContractTabs() {
  ...
  const fetchData = async () => {
    ...
    try {
      const url =
        tabValue === 0
          ? "http://13.125.78.241:8081/overseas/contracts"
          : "http://13.125.78.241:8081/overseas/contracts/cancel";
  ...
  const filterData = () => {
    ...
    const now = new Date();
    filtered = filtered.filter((item) => {
      // **수정된 부분: ordDt와 ordTmd를 결합하여 정확한 주문시간을 계산**
      const orderDate = new Date(
        `${item.ordDt}T${item.ordTmd.slice(0, 2)}:${item.ordTmd.slice(
          2,
          4
    ...
    // **수정된 부분: 주문시간 기준으로 내림차순 정렬 추가**
    filtered.sort((a, b) => {
      const dateA = new Date(
        `${a.ordDt}T${a.ordTmd.slice(0, 2)}:${a.ordTmd.slice(
          2,
          4
        )}:${a.ordTmd.slice(4, 6)}`
      );
      const dateB = new Date(
        `${b.ordDt}T${b.ordTmd.slice(0, 2)}:${b.ordTmd.slice(
          2,
          4
        )}:${b.ordTmd.slice(4, 6)}`
      );
              ...
              {filteredData.map((row, index) => (
                <tr key={index} style={{ borderBottom: "1px solid #ddd" }}>
                  ...
                  <td
                    ...
                    {Number(row.ftOrdUnpr3).toLocaleString("ko-KR", {
                      //   style: "currency",
                      currency: "USD",
                    })}
                  </td>
                  <td
                    ...
                    {Number(row.ftCcldAmt3).toLocaleString("ko-KR", {
                      //   style: "currency",
                      currency: "USD",
                    })}
                  </td>
