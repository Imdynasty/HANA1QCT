...
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": ["http://localhost:3000", "http://hana1qct.info"]}})
...
def place_domestic_order():
    try:
        ...
        if order_result is not None:
            ...
            # 'ord_dt'와 'ord_tmd'를 합쳐서 datetime 객체로 변환하고 시간대 설정
            filtered_contracts['converted_ord_tmd'] = pd.to_datetime(
                filtered_contracts['ord_dt'] + filtered_contracts['ord_tmd'], format='%Y%m%d%H%M%S'
            ).dt.tz_localize(kst)
...
def send_kakao_message(contracts):
    with open(r"/User/smin/kakao_code_friend2.json", "r") as fp:
        tokens = json.load(fp)
...
def send_kakao_overseas_message(contracts):
    with open(r"/home/ubuntu/kakao_code_friend2.json", "r") as fp:
        tokens = json.load(fp)
